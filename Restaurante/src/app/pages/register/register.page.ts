...
export class RegisterPage implements OnInit {
        ...
        if (qrData.includes('@')) {
          ...
          if (datos.length >= 3) {
            this.registerForm.patchValue({
              ...
              dni: datos[4].trim(),
              cuil:
                datos[8].slice(0, 2) + '-' + datos[4] + '-' + datos[8].slice(2),
            });
          ...
          const alert = await this.alertController.create({
            ...
            message:
              'El código QR contiene información en un formato no reconocido. ¿Desea usar estos datos como DNI?',
            buttons: [
              {
                text: 'No',
                role: 'cancel',
  ...
  async mostrarAyudaQR() {
    const alert = await this.alertController.create({
      ...
      message:
        'Si el lector QR no funciona correctamente, puedes:<br><br>' +
